...
const { createProgressBar, format, convertTime } = require("./Utils");
...
class DiscordTunes extends EventEmitter {
    ...
    async connect() {
       this.client.manager = new Manager({
        ...
        this.client.on("voiceStateUpdate", async (client, oldState, newState) => {
            this.emit("logs", `Internal event: VoiceStateUpdate, #1 ${oldState}\n\n#2 ${newState}`);
            try {
     ...
     */
     queueList(guildID, pageNum, embedColor) {
        ...
        const { queue } = player;
        const embed = new MessageEmbed().setColor(embedColor || "RANDOM");
        const multiple = 10;
     ...
     */
     async addPrevious(guildID) {
        ...
        try {
            ...
            if (res.loadType === "LOAD_FAILED") {
                this.emit("loadFailed", "Something broke while playing the track.");
                if (!player.queue.current) await player.destroy();
                throw res.exception;
            }
        } catch (e) {
